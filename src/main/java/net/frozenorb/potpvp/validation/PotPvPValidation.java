...
public final class PotPvPValidation {
    ...
    public static boolean canSendDuel(Player sender, Player target) {
        ...
        if (isInSilentMode(target)) {
            sender.sendMessage(TARGET_OFFLINE);
            return false;
        }
    ...
    public static boolean canFollowSomeone(Player player, Player target) {
        ...
        if (isInSilentMode(target) && !player.hasPermission("kore.staff")) {
            player.sendMessage(TARGET_OFFLINE);
            return false;
        }
    ...
    private boolean isInTournament(Party party) {
        TournamentHandler tournamentHandler = PotPvPND.getInstance().getTournamentHandler();
