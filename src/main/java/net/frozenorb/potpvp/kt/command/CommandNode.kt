...
class CommandNode(
    ...
    fun getUsage(realLabel: String): FancyMessage {
        ...
        flags.addAll(this.parameters.stream().filter { data -> data is FlagData }.map { data -> data as FlagData }
            .collect(Collectors.toList()))

        val parameters = mutableListOf<ParameterData>()
        parameters.addAll(this.parameters.stream().filter { data -> data is ParameterData }
            .map { data -> data as ParameterData }.collect(Collectors.toList()))
        ...
        if (parameters.isNotEmpty()) {
            for (index in parameters.indices) {
                ...
                usage.then((if (required) "<" else "[") + parameter.name + (if (parameter.wildcard) "..." else "") + (if (required) ">" else "]") + if (index != parameters.size - 1) " " else "")
                    .color(ChatColor.RED)
                usage.tooltip("${ChatColor.YELLOW}$description")
            }
        }
    ...
    fun getUsage(): FancyMessage {
        ...
        val flags = mutableListOf<FlagData>()
        flags.addAll(parameters.stream().filter { data -> data is FlagData }.map { data -> data as FlagData }
            .collect(Collectors.toList()))

        val parameters = mutableListOf<ParameterData>()
        parameters.addAll(parameters.stream().filter { data -> data is ParameterData }
            .map { data -> data as ParameterData }.collect(Collectors.toList()))
    ...
    operator fun invoke(sender: CommandSender, arguments: Arguments): Boolean {
        ...
        for (data in this.parameters) {
            if (data is FlagData) {
                ...
                if (type == null) {
                    sender.sendMessage(
                        "${ChatColor.RED}No data type found: ${
                            (data.parameterType
    ...
    fun getSubCommands(sender: CommandSender, print: Boolean): List<String> {
        ...
        if (canUse(sender)) {
            val command =
                (if (sender is Player) "/" else "") + this.getFullLabel() + " " + getUsage().toOldMessageFormat() + if (Strings.isNullOrEmpty(
                        this.description
                    )
                ) "" else "${ChatColor.GRAY} - $description"
