        ...
        for (final Game game : GameQueue.INSTANCE.getCurrentGames()) {
            toReturn.put(toReturn.size(), new Button(){
                ...
                public List<String> getDescription(Player player) {
                    ...
                    for (String line : Arrays.asList("&7&m-------------------------", "&fPlayers&7: &b" + game.getPlayers().size() + (game.getMaxPlayers() == -1 ? "" : "&7/" + game.getMaxPlayers()), "&fState&7: &b" + StringUtils.capitalize((String)game.getState().name().toLowerCase()), "&fHosted By&7: &b" + game.getHost().getDisplayName(), " ", game.getState() == GameState.STARTING ? "&7Click here to join." : "&7Click here to spectate.", "&7&m-------------------------")) {
